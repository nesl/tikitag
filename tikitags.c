...
char INIT1[] =
  { 0x6f, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x14,
0x00, 0x00, 0x08 };
char INIT2[] =
  { 0x6f, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x14,
0x04, 0x00, 0x06 };
char INIT3[] =
  { 0x6f, 0x0b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00,
0x00, 0x00, 0x06, 0xD4, 0x32, 0x05, 0x00, 0x00, 0x00 };
char INIT4[] =
  { 0x6f, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xC0,
0x00, 0x00, 0x04 };
char status[] =
...
char poll[] =
  { 0x6f, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00,
0x00, 0x00, 0x05, 0xd4, 0x60, 0x01, 0x01, 0x10 };
char get[] =
  { 0x6f, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xc0,
0x00, 0x00, 0x05 };
...
{				//seektime/(looprate+65ms)=iterations (65ms or 95ms is roughly time to run through one iteration)
  ...
  while (t < (seektime / (looprate + 95)))
      ...
      for (i = 0; i < numoftiki; i++)
	{
	  poll[6] = count;
	  if ((recvb =
	       tiki_xchange (tiki[i], poll, sizeof (poll), recvbuf,
			     sizeof (recvbuf))) < 0)
	    {
	      perror ("xchange");
	      return (-1);
	  ...
	  get[14] = recvbuf[11];
	  if ((recvb =
	       tiki_xchange (tiki[i], get, sizeof (get), recvbuf,
			     sizeof (recvbuf))) < 0)
	    {
	      perror ("xchange");
	    }
